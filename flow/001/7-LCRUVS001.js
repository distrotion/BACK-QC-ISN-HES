...
router.get('/CHECK-LCRUVS001', async (req, res) => {

  return res.json(LCRUVS001db['PO']);
});


router.post('/LCRUVS001db', async (req, res) => {
  //-------------------------------------
  // console.log('--LCRUVS001db--');
...
router.post('/GETINtoLCRUVS001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoLCRUVS001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/LCRUVS001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (let i = 0; i < LCRUVS001db['ItemPickcode'].length; i++) {
    if (LCRUVS001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = LCRUVS001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    LCRUVS001db['inspectionItem'] = ITEMSS;
    LCRUVS001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': LCRUVS001db["PO"], 'CP': LCRUVS001db["CP"], 'ITEMs': LCRUVS001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": LCRUVS001db['inspectionItem'] });
      ...
      for (let i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            LCRUVS001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            LCRUVS001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            LCRUVS001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              LCRUVS001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (let j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              LCRUVS001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          LCRUVS001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          LCRUVS001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          LCRUVS001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          LCRUVS001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          finddbbuffer = null;
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
                        axios.post('http://127.0.0.1:16010/LCRUVS001-feedback', { "PO": LCRUVS001db['PO'], "ITEMs": LCRUVS001db['inspectionItem'] }).catch(() => {});
...
router.post('/LCRUVS001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    LCRUVS001db['GAPnameListdata'] = graph[0];//confirmdata
    LCRUVS001db['GAP'] = LCRUVS001db['GAPnameListdata'][`GT${LCRUVS001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/LCRUVS001-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--LCRUVS001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/LCRUVS001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = LCRUVS001db['preview'][0]

    if (LCRUVS001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = LCRUVS001db['preview'][0]

      pushdata['V5'] = LCRUVS001db['GAP'];
      pushdata['V1'] = `${LCRUVS001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      LCRUVS001db['confirmdata'].push(pushdata);
      LCRUVS001db['preview'] = [];
      finddbbuffer = null;
      output = 'OK';
      LCRUVS001db['GAP'] = LCRUVS001db['GAPnameListdata'][`GT${LCRUVS001db['confirmdata'].length + 1}`]

    } else if (LCRUVS001db['RESULTFORMAT'] === 'Number') {

      let pushdata = LCRUVS001db['preview'][0]

      pushdata['V5'] = LCRUVS001db['confirmdata'].length + 1
      pushdata['V1'] = `${LCRUVS001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/LCRUVS001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      LCRUVS001db["PCSleft"] = `${parseInt(LCRUVS001db["PCS"]) - oblist.length}`;
      if (LCRUVS001db['RESULTFORMAT'] === 'Number' || LCRUVS001db['RESULTFORMAT'] === 'Text' || LCRUVS001db['RESULTFORMAT'] === 'Graph') {
        ...
        LCRUVS001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        LCRUVS001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(LCRUVS001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (LCRUVS001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (LCRUVS001db['INTERSEC'] !== '') {
                core = parseFloat(LCRUVS001db['INTERSEC'])
              } else {
      ...
      LCRUVS001db["ITEMleftUNIT"] = '';
      LCRUVS001db["ITEMleftVALUE"] = '';
    }
...
router.post('/LCRUVS001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/LCRUVS001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/LCRUVS001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = LCRUVS001db['confirmdata'].length
    if (all > 0) {
      LCRUVS001db['confirmdata'].pop();
    }
...
router.post('/LCRUVS001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--LCRUVS001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (LCRUVS001db['RESULTFORMAT'] === 'Number' || LCRUVS001db['RESULTFORMAT'] === 'Text') {

    LCRUVS001db["value"] = [];
    console.log(LCRUVS001db["value"]);
    for (let i = 0; i < LCRUVS001db['confirmdata'].length; i++) {
      LCRUVS001db["value"].push({
        "PO1": LCRUVS001db["inspectionItemNAME"],
        "PO2": LCRUVS001db['confirmdata'][i]['V1'],
        "PO3": LCRUVS001db['confirmdata'][i]['V2'],
        "PO4": LCRUVS001db['confirmdata'][i]['V3'],
        "PO5": LCRUVS001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (LCRUVS001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (let i = 0; i < LCRUVS001db["value"].length; i++) {
        mean01.push(parseFloat(LCRUVS001db["value"][i]["PO3"]));
        mean02.push(parseFloat(LCRUVS001db["value"][i]["PO5"]));
      }
      ...
      LCRUVS001db["value"].push({
        "PO1": 'Mean',
        "PO2": LCRUVS001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": LCRUVS001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (LCRUVS001db['RESULTFORMAT'] === 'OCR' || LCRUVS001db['RESULTFORMAT'] === 'Picture') {

  } else if (LCRUVS001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (let i = 0; i < LCRUVS001db['confirmdata'].length; i++) {
      LCRUVS001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": LCRUVS001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (LCRUVS001db['RESULTFORMAT'] === 'Number' ||
    LCRUVS001db['RESULTFORMAT'] === 'Text' ||
    LCRUVS001db['RESULTFORMAT'] === 'OCR' ||
    LCRUVS001db['RESULTFORMAT'] === 'Picture' || LCRUVS001db['RESULTFORMAT'] === 'Graph') {
        axios.post('http://127.0.0.1:16010/FINISHtoDB', LCRUVS001db)
          .then(() => axios.post('http://127.0.0.1:16010/LCRUVS001-feedback',
            { "PO": LCRUVS001db['PO'], "ITEMs": LCRUVS001db['inspectionItem'] }
          ).catch(() => {}))
          .catch(() => {});
        LCRUVS001db['confirmdata'] = [];
        LCRUVS001db['value'] = [];
