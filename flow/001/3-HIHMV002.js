...
router.get('/CHECK-HIHMV002', async (req, res) => {

  return res.json(HIHMV002db['PO']);
});


router.post('/HIHMV002db', async (req, res) => {
  //-------------------------------------
  // console.log('--HIHMV002db--');
...
router.post('/GETINtoHIHMV002', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHIHMV002--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIHMV002-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HIHMV002db['inspectionItem'] = ITEMSS;
    HIHMV002db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HIHMV002db["PO"], 'CP': HIHMV002db["CP"], 'ITEMs': HIHMV002db['inspectionItem'] };
    //-------------------------------------
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HIHMV002db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HIHMV002db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HIHMV002db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HIHMV002db['GAPnameList'].push(graph[k]['NO']);
            }
          }
          ...
          HIHMV002db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HIHMV002db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
...
router.post('/HIHMV002-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HIHMV002db['GAPnameListdata'] = graph[0];//confirmdata
...
router.post('/HIHMV002-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIHMV002-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HIHMV002db['preview'][0]

    if (HIHMV002db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HIHMV002db['preview'][0]

      pushdata['V5'] = HIHMV002db['GAP'];
      pushdata['V1'] = `${HIHMV002db['confirmdata'].length + 1}:${pushdata['V1']}`;
    ...
    } else if (HIHMV002db['RESULTFORMAT'] === 'Number') {

      let pushdata = HIHMV002db['preview'][0]

      pushdata['V5'] = HIHMV002db['confirmdata'].length + 1
      pushdata['V1'] = `${HIHMV002db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HIHMV002-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  HIHMV002db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/HIHMV002-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      if (HIHMV002db['RESULTFORMAT'] === 'Number' || HIHMV002db['RESULTFORMAT'] === 'Text' || HIHMV002db['RESULTFORMAT'] === 'Graph') {
        ...
        HIHMV002db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HIHMV002db["ITEMleftVALUE"] = ITEMleftVALUEout;
      ...
      if ((parseInt(HIHMV002db["PCS"]) - oblist.length) == 0) {
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HIHMV002db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HIHMV002db['INTERSEC'] !== '') {
                core = parseFloat(HIHMV002db['INTERSEC'])
              } else {
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:16010/JUDEMENT",{"PO":HIHMV002db["PO"],"CP":HIHMV002db["CP"]})
      ...
      HIHMV002db["ITEMleftUNIT"] = '';
      HIHMV002db["ITEMleftVALUE"] = '';
    }
...
router.post('/HIHMV002-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIHMV002-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HIHMV002-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HIHMV002db['confirmdata'].length
    if (all > 0) {
...
router.post('/HIHMV002-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV002-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';
  ...
  if (HIHMV002db['RESULTFORMAT'] === 'Number' || HIHMV002db['RESULTFORMAT'] === 'Text') {
    ...
    for (i = 0; i < HIHMV002db['confirmdata'].length; i++) {
      HIHMV002db["value"].push({
        ...
        "PO3": HIHMV002db['confirmdata'][i]['V2'],
        "PO4": HIHMV002db['confirmdata'][i]['V3'],
        "PO5": HIHMV002db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HIHMV002db["value"].length > 0) {
      ...
      HIHMV002db["value"].push({
        "PO1": 'Mean',
        "PO2": HIHMV002db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HIHMV002db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HIHMV002db['RESULTFORMAT'] === 'OCR' || HIHMV002db['RESULTFORMAT'] === 'Picture') {
    ...
    for (i = 0; i < HIHMV002db['confirmdata'].length; i++) {
      HIHMV002db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HIHMV002db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HIHMV002db['RESULTFORMAT'] === 'Number' ||
