...
router.get('/CHECK-HIMICM001', async (req, res) => {

  return res.json(HIMICM001db['PO']);
});


router.post('/HIMICM001db', async (req, res) => {
  //-------------------------------------
  // console.log('--HIMICM001db--');
...
router.post('/GETINtoHIMICM001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHIMICM001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIMICM001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (let i = 0; i < HIMICM001db['ItemPickcode'].length; i++) {
    if (HIMICM001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HIMICM001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HIMICM001db['inspectionItem'] = ITEMSS;
    HIMICM001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HIMICM001db["PO"], 'CP': HIMICM001db["CP"], 'ITEMs': HIMICM001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HIMICM001db['inspectionItem'] });
      ...
      for (let i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HIMICM001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HIMICM001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HIMICM001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HIMICM001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (let j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HIMICM001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HIMICM001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HIMICM001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HIMICM001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HIMICM001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          finddbbuffer = null;
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
                        axios.post('http://127.0.0.1:16010/HIMICM001-feedback', { "PO": HIMICM001db['PO'], "ITEMs": HIMICM001db['inspectionItem'] }).catch(() => {});
...
router.post('/HIMICM001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HIMICM001db['GAPnameListdata'] = graph[0];//confirmdata
    HIMICM001db['GAP'] = HIMICM001db['GAPnameListdata'][`GT${HIMICM001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HIMICM001-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--HIMICM001-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIMICM001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HIMICM001db['preview'][0]

    if (HIMICM001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HIMICM001db['preview'][0]

      pushdata['V5'] = HIMICM001db['GAP'];
      pushdata['V1'] = `${HIMICM001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      HIMICM001db['confirmdata'].push(pushdata);
      HIMICM001db['preview'] = [];
      finddbbuffer = null;
      output = 'OK';
      HIMICM001db['GAP'] = HIMICM001db['GAPnameListdata'][`GT${HIMICM001db['confirmdata'].length + 1}`]

    } else if (HIMICM001db['RESULTFORMAT'] === 'Number') {

      let pushdata = HIMICM001db['preview'][0]

      pushdata['V5'] = HIMICM001db['confirmdata'].length + 1
      pushdata['V1'] = `${HIMICM001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HIMICM001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HIMICM001db["PCSleft"] = `${parseInt(HIMICM001db["PCS"]) - oblist.length}`;
      if (HIMICM001db['RESULTFORMAT'] === 'Number' || HIMICM001db['RESULTFORMAT'] === 'Text' || HIMICM001db['RESULTFORMAT'] === 'Graph') {
        ...
        HIMICM001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HIMICM001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HIMICM001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HIMICM001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HIMICM001db['INTERSEC'] !== '') {
                core = parseFloat(HIMICM001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HIMICM001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HIMICM001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (let i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:16010/JUDEMENT",{"PO":HIMICM001db["PO"],"CP":HIMICM001db["CP"]})
      ...
      HIMICM001db["ITEMleftUNIT"] = '';
      HIMICM001db["ITEMleftVALUE"] = '';
    }
...
router.post('/HIMICM001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIMICM001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HIMICM001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HIMICM001db['confirmdata'].length
    if (all > 0) {
      HIMICM001db['confirmdata'].pop();
    }
...
router.post('/HIMICM001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HIMICM001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HIMICM001db['RESULTFORMAT'] === 'Number' || HIMICM001db['RESULTFORMAT'] === 'Text') {

    HIMICM001db["value"] = [];
    console.log(HIMICM001db["value"]);
    for (let i = 0; i < HIMICM001db['confirmdata'].length; i++) {
      HIMICM001db["value"].push({
        "PO1": HIMICM001db["inspectionItemNAME"],
        "PO2": HIMICM001db['confirmdata'][i]['V1'],
        "PO3": HIMICM001db['confirmdata'][i]['V2'],
        "PO4": HIMICM001db['confirmdata'][i]['V3'],
        "PO5": HIMICM001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HIMICM001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (let i = 0; i < HIMICM001db["value"].length; i++) {
        mean01.push(parseFloat(HIMICM001db["value"][i]["PO3"]));
        mean02.push(parseFloat(HIMICM001db["value"][i]["PO5"]));
      }
      ...
      HIMICM001db["value"].push({
        "PO1": 'Mean',
        "PO2": HIMICM001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HIMICM001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HIMICM001db['RESULTFORMAT'] === 'OCR' || HIMICM001db['RESULTFORMAT'] === 'Picture') {

  } else if (HIMICM001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (let i = 0; i < HIMICM001db['confirmdata'].length; i++) {
      HIMICM001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HIMICM001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HIMICM001db['RESULTFORMAT'] === 'Number' ||
    HIMICM001db['RESULTFORMAT'] === 'Text' ||
    HIMICM001db['RESULTFORMAT'] === 'OCR' ||
    HIMICM001db['RESULTFORMAT'] === 'Picture' || HIMICM001db['RESULTFORMAT'] === 'Graph') {
        axios.post('http://127.0.0.1:16010/FINISHtoDB', HIMICM001db)
          .then(() => axios.post('http://127.0.0.1:16010/HIMICM001-feedback',
            { "PO": HIMICM001db['PO'], "ITEMs": HIMICM001db['inspectionItem'] }
          ).catch(() => {}))
          .catch(() => {});
        HIMICM001db['confirmdata'] = [];
        HIMICM001db['value'] = [];
