...
router.get('/CHECK-HIMIC001', async (req, res) => {

  return res.json(HIMIC001db['PO']);
});


router.post('/HIMIC001db', async (req, res) => {
  //-------------------------------------
  // console.log('--HIMIC001db--');
...
router.post('/GETINtoHIMIC001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHIMIC001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIMIC001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (let i = 0; i < HIMIC001db['ItemPickcode'].length; i++) {
    if (HIMIC001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HIMIC001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HIMIC001db['inspectionItem'] = ITEMSS;
    HIMIC001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HIMIC001db["PO"], 'CP': HIMIC001db["CP"], 'ITEMs': HIMIC001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HIMIC001db['inspectionItem'] });
      ...
      for (let i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HIMIC001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HIMIC001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HIMIC001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HIMIC001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (let j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HIMIC001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HIMIC001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HIMIC001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HIMIC001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HIMIC001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          finddbbuffer = null;
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
                        axios.post('http://127.0.0.1:16000/HIMIC001-feedback', { "PO": HIMIC001db['PO'], "ITEMs": HIMIC001db['inspectionItem'] }).catch(() => {});
...
router.post('/HIMIC001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HIMIC001db['GAPnameListdata'] = graph[0];//confirmdata
    HIMIC001db['GAP'] = HIMIC001db['GAPnameListdata'][`GT${HIMIC001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HIMIC001-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIMIC001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HIMIC001db['preview'][0]

    if (HIMIC001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HIMIC001db['preview'][0]

      pushdata['V5'] = HIMIC001db['GAP'];
      pushdata['V1'] = `${HIMIC001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      HIMIC001db['confirmdata'].push(pushdata);
      HIMIC001db['preview'] = [];
      finddbbuffer = null;
      output = 'OK';
      HIMIC001db['GAP'] = HIMIC001db['GAPnameListdata'][`GT${HIMIC001db['confirmdata'].length + 1}`]

    } else if (HIMIC001db['RESULTFORMAT'] === 'Number') {

      let pushdata = HIMIC001db['preview'][0]

      pushdata['V5'] = HIMIC001db['confirmdata'].length + 1
      pushdata['V1'] = `${HIMIC001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HIMIC001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HIMIC001db["PCSleft"] = `${parseInt(HIMIC001db["PCS"]) - oblist.length}`;
      if (HIMIC001db['RESULTFORMAT'] === 'Number' || HIMIC001db['RESULTFORMAT'] === 'Text' || HIMIC001db['RESULTFORMAT'] === 'Graph') {
        ...
        HIMIC001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HIMIC001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HIMIC001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HIMIC001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HIMIC001db['INTERSEC'] !== '') {
                core = parseFloat(HIMIC001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HIMIC001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HIMIC001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (let i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
      ...
      HIMIC001db["ITEMleftUNIT"] = '';
      HIMIC001db["ITEMleftVALUE"] = '';
    }
...
router.post('/HIMIC001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIMIC001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HIMIC001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HIMIC001db['confirmdata'].length
    if (all > 0) {
      HIMIC001db['confirmdata'].pop();
    }
...
router.post('/HIMIC001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HIMIC001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HIMIC001db['RESULTFORMAT'] === 'Number' || HIMIC001db['RESULTFORMAT'] === 'Text') {

    HIMIC001db["value"] = [];
    for (let i = 0; i < HIMIC001db['confirmdata'].length; i++) {
      HIMIC001db["value"].push({
        "PO1": HIMIC001db["inspectionItemNAME"],
        "PO2": HIMIC001db['confirmdata'][i]['V1'],
        "PO3": HIMIC001db['confirmdata'][i]['V2'],
        "PO4": HIMIC001db['confirmdata'][i]['V3'],
        "PO5": HIMIC001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HIMIC001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (let i = 0; i < HIMIC001db["value"].length; i++) {
        mean01.push(parseFloat(HIMIC001db["value"][i]["PO3"]));
        mean02.push(parseFloat(HIMIC001db["value"][i]["PO5"]));
      }
      ...
      HIMIC001db["value"].push({
        "PO1": 'Mean',
        "PO2": HIMIC001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HIMIC001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HIMIC001db['RESULTFORMAT'] === 'OCR' || HIMIC001db['RESULTFORMAT'] === 'Picture') {

  } else if (HIMIC001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (let i = 0; i < HIMIC001db['confirmdata'].length; i++) {
      HIMIC001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HIMIC001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HIMIC001db['RESULTFORMAT'] === 'Number' ||
    HIMIC001db['RESULTFORMAT'] === 'Text' ||
    HIMIC001db['RESULTFORMAT'] === 'OCR' ||
    HIMIC001db['RESULTFORMAT'] === 'Picture' || HIMIC001db['RESULTFORMAT'] === 'Graph') {
        axios.post('http://127.0.0.1:16000/FINISHtoDB', HIMIC001db).catch(() => {});

  }
  //-------------------------------------
  return res.json(HIMIC001db);
});
