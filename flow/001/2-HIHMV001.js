...
router.get('/CHECK-HIHMV001', async (req, res) => {

  return res.json(HIHMV001db['PO']);
});


router.post('/HIHMV001db', async (req, res) => {
  //-------------------------------------
  // console.log('--HIHMV001db--');
...
router.post('/GETINtoHIHMV001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHIHMV001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIHMV001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (let i = 0; i < HIHMV001db['ItemPickcode'].length; i++) {
    if (HIHMV001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HIHMV001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HIHMV001db['inspectionItem'] = ITEMSS;
    HIHMV001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HIHMV001db["PO"], 'CP': HIHMV001db["CP"], 'ITEMs': HIHMV001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HIHMV001db['inspectionItem'] });
      ...
      for (let i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HIHMV001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HIHMV001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HIHMV001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HIHMV001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (let j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HIHMV001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HIHMV001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HIHMV001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HIHMV001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HIHMV001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          finddbbuffer = null;
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
                        axios.post('http://127.0.0.1:16010/HIHMV001-feedback', { "PO": HIHMV001db['PO'], "ITEMs": HIHMV001db['inspectionItem'] }).catch(() => {});
...
router.post('/HIHMV001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HIHMV001db['GAPnameListdata'] = graph[0];//confirmdata
    HIHMV001db['GAP'] = HIHMV001db['GAPnameListdata'][`GT${HIHMV001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/HIHMV001-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIHMV001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HIHMV001db['preview'][0]

    if (HIHMV001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HIHMV001db['preview'][0]

      pushdata['V5'] = HIHMV001db['GAP'];
      pushdata['V1'] = `${HIHMV001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      HIHMV001db['confirmdata'].push(pushdata);
      HIHMV001db['preview'] = [];
      finddbbuffer = null;
      output = 'OK';
      HIHMV001db['GAP'] = HIHMV001db['GAPnameListdata'][`GT${HIHMV001db['confirmdata'].length + 1}`]

    } else if (HIHMV001db['RESULTFORMAT'] === 'Number') {

      let pushdata = HIHMV001db['preview'][0]

      pushdata['V5'] = HIHMV001db['confirmdata'].length + 1
      pushdata['V1'] = `${HIHMV001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/HIHMV001-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  HIHMV001db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/HIHMV001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HIHMV001db["PCSleft"] = `${parseInt(HIHMV001db["PCS"]) - oblist.length}`;
      if (HIHMV001db['RESULTFORMAT'] === 'Number' || HIHMV001db['RESULTFORMAT'] === 'Text' || HIHMV001db['RESULTFORMAT'] === 'Graph') {
        ...
        HIHMV001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HIHMV001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HIHMV001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HIHMV001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HIHMV001db['INTERSEC'] !== '') {
                core = parseFloat(HIHMV001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HIHMV001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HIHMV001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (let i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:16010/JUDEMENT",{"PO":HIHMV001db["PO"],"CP":HIHMV001db["CP"]})
      ...
      HIHMV001db["ITEMleftUNIT"] = '';
      HIHMV001db["ITEMleftVALUE"] = '';
    }
...
router.post('/HIHMV001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/HIHMV001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/HIHMV001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HIHMV001db['confirmdata'].length
    if (all > 0) {
      HIHMV001db['confirmdata'].pop();
    }
...
router.post('/HIHMV001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HIHMV001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HIHMV001db['RESULTFORMAT'] === 'Number' || HIHMV001db['RESULTFORMAT'] === 'Text') {

    HIHMV001db["value"] = [];
    for (let i = 0; i < HIHMV001db['confirmdata'].length; i++) {
      HIHMV001db["value"].push({
        "PO1": HIHMV001db["inspectionItemNAME"],
        "PO2": HIHMV001db['confirmdata'][i]['V1'],
        "PO3": HIHMV001db['confirmdata'][i]['V2'],
        "PO4": HIHMV001db['confirmdata'][i]['V3'],
        "PO5": HIHMV001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HIHMV001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (let i = 0; i < HIHMV001db["value"].length; i++) {
        mean01.push(parseFloat(HIHMV001db["value"][i]["PO3"]));
        mean02.push(parseFloat(HIHMV001db["value"][i]["PO5"]));
      }
      ...
      HIHMV001db["value"].push({
        "PO1": 'Mean',
        "PO2": HIHMV001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HIHMV001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HIHMV001db['RESULTFORMAT'] === 'OCR' || HIHMV001db['RESULTFORMAT'] === 'Picture') {

  } else if (HIHMV001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (let i = 0; i < HIHMV001db['confirmdata'].length; i++) {
      HIHMV001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HIHMV001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HIHMV001db['RESULTFORMAT'] === 'Number' ||
    HIHMV001db['RESULTFORMAT'] === 'Text' ||
    HIHMV001db['RESULTFORMAT'] === 'OCR' ||
    HIHMV001db['RESULTFORMAT'] === 'Picture' || HIHMV001db['RESULTFORMAT'] === 'Graph') {
        axios.post('http://127.0.0.1:16010/FINISHtoDB', HIHMV001db)
          .then(() => axios.post('http://127.0.0.1:16010/HIHMV001-feedback',
            { "PO": HIHMV001db['PO'], "ITEMs": HIHMV001db['inspectionItem'] }
          ).catch(() => {}))
          .catch(() => {});
        HIHMV001db['confirmdata'] = [];
        HIHMV001db['value'] = [];
