...
router.get('/CHECK-TPGMCSCPR', async (req, res) => {
...
router.post('/GETINtoTPGMCSCPR', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoTPGMCSCPR--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TPGMCSCPR-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPR-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    TPGMCSCPRdb['inspectionItem'] = ITEMSS;
    TPGMCSCPRdb['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': TPGMCSCPRdb["PO"], 'CP': TPGMCSCPRdb["CP"], 'ITEMs': TPGMCSCPRdb['inspectionItem'] };
    //-------------------------------------
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            TPGMCSCPRdb["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            TPGMCSCPRdb["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            TPGMCSCPRdb['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              TPGMCSCPRdb['GAPnameList'].push(graph[k]['NO']);
            }
          }
          ...
          TPGMCSCPRdb["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          TPGMCSCPRdb["PCS"] = findcp[0]['FINAL'][i]['PCS'];
...
router.post('/TPGMCSCPR-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPR-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    TPGMCSCPRdb['GAPnameListdata'] = graph[0];//confirmdata
...
router.post('/TPGMCSCPR-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--TPGMCSCPR-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TPGMCSCPR-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPR-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = TPGMCSCPRdb['preview'][0]

    if (TPGMCSCPRdb['RESULTFORMAT'] === 'Graph') {
      let pushdata = TPGMCSCPRdb['preview'][0]

      pushdata['V5'] = TPGMCSCPRdb['GAP'];
      pushdata['V1'] = `${TPGMCSCPRdb['confirmdata'].length + 1}:${pushdata['V1']}`;
    ...
    } else if (TPGMCSCPRdb['RESULTFORMAT'] === 'Number') {

      let pushdata = TPGMCSCPRdb['preview'][0]

      pushdata['V5'] = TPGMCSCPRdb['confirmdata'].length + 1
      pushdata['V1'] = `${TPGMCSCPRdb['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/TPGMCSCPR-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPR-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      if (TPGMCSCPRdb['RESULTFORMAT'] === 'Number' || TPGMCSCPRdb['RESULTFORMAT'] === 'Text' || TPGMCSCPRdb['RESULTFORMAT'] === 'Graph') {
        ...
        TPGMCSCPRdb["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        TPGMCSCPRdb["ITEMleftVALUE"] = ITEMleftVALUEout;
      ...
      if ((parseInt(TPGMCSCPRdb["PCS"]) - oblist.length) == 0) {
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (TPGMCSCPRdb['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (TPGMCSCPRdb['INTERSEC'] !== '') {
                core = parseFloat(TPGMCSCPRdb['INTERSEC'])
              } else {
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:16010/JUDEMENT",{"PO":TPGMCSCPRdb["PO"],"CP":TPGMCSCPRdb["CP"]})
      ...
      TPGMCSCPRdb["ITEMleftUNIT"] = '';
      TPGMCSCPRdb["ITEMleftVALUE"] = '';
    }
...
router.post('/TPGMCSCPR-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPRfromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TPGMCSCPR-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPRfromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/TPGMCSCPR-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPRfromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = TPGMCSCPRdb['confirmdata'].length
    if (all > 0) {
...
router.post('/TPGMCSCPR-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--TPGMCSCPR-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (TPGMCSCPRdb['RESULTFORMAT'] === 'Number' || TPGMCSCPRdb['RESULTFORMAT'] === 'Text') {
    ...
    for (i = 0; i < TPGMCSCPRdb['confirmdata'].length; i++) {
      TPGMCSCPRdb["value"].push({
        ...
        "PO3": TPGMCSCPRdb['confirmdata'][i]['V2'],
        "PO4": TPGMCSCPRdb['confirmdata'][i]['V3'],
        "PO5": TPGMCSCPRdb['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (TPGMCSCPRdb["value"].length > 0) {
      ...
      TPGMCSCPRdb["value"].push({
        "PO1": 'Mean',
        "PO2": TPGMCSCPRdb['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": TPGMCSCPRdb['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (TPGMCSCPRdb['RESULTFORMAT'] === 'OCR' || TPGMCSCPRdb['RESULTFORMAT'] === 'Picture') {
    ...
    for (i = 0; i < TPGMCSCPRdb['confirmdata'].length; i++) {
      TPGMCSCPRdb["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": TPGMCSCPRdb['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (TPGMCSCPRdb['RESULTFORMAT'] === 'Number' ||
